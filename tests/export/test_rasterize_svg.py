...
from grafix.export import rasterize_svg
...
def test_default_png_output_path_uses_data_dir_and_script_stem():
    ...
    path = rasterize_svg.default_png_output_path(draw)
...
def test_png_output_size_scales_canvas_by_png_scale():
    expected = (
        int(300 * float(rasterize_svg.PNG_SCALE)),
        int(300 * float(rasterize_svg.PNG_SCALE)),
    )
    assert rasterize_svg.png_output_size((300, 300)) == expected
...
def test_rasterize_svg_to_png_invokes_resvg_with_resized_svg(tmp_path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(rasterize_svg.subprocess, "run", fake_run)

    path = rasterize_svg.rasterize_svg_to_png(src_svg, out_png, output_size=(1200, 1200))
    assert path == out_png
...
def test_rasterize_svg_to_png_raises_when_resvg_is_missing(tmp_path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.setattr(rasterize_svg.subprocess, "run", missing)

    with pytest.raises(RuntimeError, match="resvg が見つかりません"):
        rasterize_svg.rasterize_svg_to_png(src_svg, out_png, output_size=(10, 10))
