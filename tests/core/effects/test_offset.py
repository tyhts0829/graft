...
@primitive
def offset_test_segment_xy() -> RealizedGeometry:
    """xy 平面上の 2 点線分を返す。"""
...
@primitive
def offset_test_segment_xz() -> RealizedGeometry:
...
def test_offset_distance_zero_is_noop() -> None:
    g = G.offset_test_segment_xy()
    out = E.offset(distance=0.0)(g)
...
def test_offset_expands_bounds_xy() -> None:
    g = G.offset_test_segment_xy()
    out = E.offset(distance=0.1, segments_per_circle=4, join="round")(g)
    realized = realize(out)
...
def test_offset_keep_original_appends_input() -> None:
    g = G.offset_test_segment_xy()
    out = E.offset(distance=0.1, segments_per_circle=4, keep_original=True)(g)
    realized = realize(out)
...
def test_offset_preserves_plane_for_xz_segment() -> None:
    g = G.offset_test_segment_xz()
    out = E.offset(distance=0.1, segments_per_circle=4)(g)
    realized = realize(out)
