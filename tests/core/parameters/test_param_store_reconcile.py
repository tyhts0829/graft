...
from src.core.parameters.context import parameter_context
from src.core.parameters.frame_params import FrameParamRecord
from src.core.parameters.layer_style import LAYER_STYLE_OP, layer_style_records
...
def test_reconcile_migrates_state_and_hides_stale_group_in_gui_snapshot():
    ...
    original = ParamStore()
    for rec in _polyhedron_records(old_site_id):
        state = original.ensure_state(rec.key, base_value=rec.base)
        state.override = True
        if rec.key.arg == "center":
            state.ui_value = (12.0, 34.0, 56.0)
...
def test_reconcile_does_not_migrate_when_ambiguous():
    ...
    original = ParamStore()
    for rec in _polyhedron_records("old-a"):
        state = original.ensure_state(rec.key, base_value=rec.base)
        state.override = True
        if rec.key.arg == "type_index":
            state.ui_value = 1
        original.set_meta(rec.key, rec.meta)
    for rec in _polyhedron_records("old-b"):
        state = original.ensure_state(rec.key, base_value=rec.base)
        state.override = True
        if rec.key.arg == "type_index":
            state.ui_value = 4
...
def test_prune_removes_stale_effect_steps_and_unused_chain_ordinals():
    ...
    old_state.override = True
    old_state.ui_value = 90.0
...
def test_reconcile_handles_g_polyhedron_kwargs_edit_without_gui_duplication():
    ...
    draw_v1 = _compile_draw(
        """
from src.api.primitives import G
