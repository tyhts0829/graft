...
- [x] 現状確認
  - 対象: `src/parameters/store.py`（`snapshot/store_frame_params/to_json/from_json`）、`src/parameters/context.py`（`parameter_context`）、`tests/parameters/test_resolver.py` など snapshot 依存部。
...
- [x] マージ処理
  - 対象: `ParamStore.store_frame_params`。
...
- [x] テスト追加/更新
  - 対象: `tests/parameters/test_resolver.py`、新規 `tests/parameters/test_store_meta.py`。
  - 内容: meta を含む JSON round-trip、`snapshot()` の内容・順序（ordinal 単調増加）、context で新スナップショットが参照できること、旧 snapshot 型を前提にしたテストの差し替え。
- [x] ドキュメント更新
  - 対象: `docs/parameter_gui_impl_plan.md` フェーズ 1 節。
