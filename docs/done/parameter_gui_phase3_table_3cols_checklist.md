...
- 対象 kind: `float` / `int` / `vec3` / `bool` / `str` / `choice`（未知 kind は例外）
- レイアウト: 4 列テーブル
  - 1: label
  - 2: control（kind に応じたウィジェット）
  - 3: min-max（`ui_min` / `ui_max`。`float/int/vec3` のみ。`bool/str/choice` は空）
  - 4: cc / override（`cc_key` / `override`。`float/int/vec3` のみ。`bool/str/choice` は空）
...
- [x] 1 行ぶんの描画関数を追加（デバッグしやすくする）
  ...
  - [x] column 3: min-max を描画
    ...
    - [x] `bool/str/choice`: 描画しない（列を空にする）
