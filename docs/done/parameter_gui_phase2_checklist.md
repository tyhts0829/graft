...
- [x] データモデル設計  
  ...
  - 内容: `ParameterRow` dataclass（label/op/arg/kind/ui_value/ui_min/ui_max/choices/cc_key/override/ordinal）を定義。kind は choice に一本化し、列挙は `choices` で扱う。last_seen は採用しない。
- [x] Row 生成ロジック  
...
- [x] ParamStore 連携ヘルパ  
  ...
  - テスト: `tests/parameters/test_parameter_updates.py` で override 切替と値更新を確認。
- [ ] ドキュメント更新  
