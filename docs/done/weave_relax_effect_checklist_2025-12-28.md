...
- [x] 「固定点（fixed）」の決め方（入力 1 個しか使えない制約あり）:
...
- [x] 次元: 3D（`(x,y,z)`）で緩和してよいか（`weave` 出力は平面上なので 3D でも平面を保つ想定）;OK
- [x] `weave` 側の扱い:
...
- [x] `E.relax(...)` が effect として登録され、`grafix.api.__init__.pyi` のスタブにも反映される
- [x] `weave` は一切変更されない（`git diff` 上、`src/grafix/core/effects/weave.py` に差分無し）
- [x] 最小のユニットテストが追加される（緩和が効く/固定点が動かない/共有点が崩れない等）

## 2) API 仕様（案）

- effect: `relax(inputs, *, relaxation_iterations: int = 15, step: float = 0.125)`
- meta（案）:
...
- [x] `src/grafix/core/effects/relax.py` を新規作成（effect 本体）
  - [x] 入力 `RealizedGeometry` から「ノード共有」を復元する（方針 0-B）
    - [x] 共有判定は座標の完全一致（`(x,y,z)` のタプル）で同一点を同ノードに束ねる
    - [x] エッジは各ポリラインの隣接頂点から生成し、重複エッジは除去
  - [x] `fixed` マスクを作る（次数 `!=2` + 連結成分の min/max）
  - [x] `elastic_relaxation_nb` 相当の緩和ループを `relax.py` 内に実装し、座標を更新（`weave` と共通化しない）
  - [x] 元のポリライン構造（頂点数/offsets）を保ったまま出力へ戻す
- [x] `src/grafix/api/effects.py` に `from grafix.core.effects import relax as _effect_relax` を追加（登録用）
- [x] スタブ更新: `tools/gen_g_stubs.py` の生成結果で `src/grafix/api/__init__.pyi` を更新

## 4) テスト（最小の安全柵）

- [x] `tests/core/effects/test_relax.py`（新規）
  - [x] 反復 0 のとき入力と一致する（早期 return の確認）
  - [x] 固定されない点が移動する
  - [x] 共有点の出力座標が一致し続ける（方針 0-B）
- [x] `pytest -q tests/stubs/test_api_stub_sync.py` が通る（スタブ同期）

## 5) 実行コマンド（ローカル確認）

- [x] `PYTHONPATH=src pytest -q tests/core/effects/test_relax.py`
- [x] `PYTHONPATH=src pytest -q tests/stubs/test_api_stub_sync.py`
- [ ] `ruff check src/grafix/core/effects/relax.py`（この環境では ruff 未導入）
- [x] `mypy src/grafix/core/effects/relax.py`

## 追加で事前確認したほうがいい点 / 追加提案（気づいたら追記）

- [ ] 近似共有（epsilon）を導入するか（完全一致だと「ほぼ同一点」を束ねない）
