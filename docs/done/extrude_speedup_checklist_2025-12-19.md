...
- 高速化のコード変更は実施済み（このファイルは判断材料 + 実施ログ）。
...
- [x] **Python ループ + 小配列の大量生成**
  - 対応: 接続エッジを per-vertex の小配列生成から脱却（ベクトル化 + 1 回確保へ）。
- [x] **`out_lines`（小配列リスト）→ `np.concatenate`**
  - 対応: 2 パス（count→fill）で `out_coords/out_offsets` を 1 度だけ確保して詰める。
- [x] **dtype 変換（float64 経由）**
  - 対応: 全点の `float64` 配列を作らず、中心計算のみを最小限の `float64` に限定（残りは float32 で処理）。
- [x] **subdivide の反復確保**
  - 対応: Numba 実装で 1 回確保（均等分割）に置き換え。

## 2) 高速化アイデア（仕様不変）

### 2.1 まず効く（推奨 / 低リスク）

- [x] **2 パス化（count→fill）で 1 回だけ確保する**
...
- [x] **`center_mode="auto"` の式変形で余計な配列を減らす**
...
- [x] **subdivide の高速化**
  - 中点挿入の結果は「各セグメントを `2^subdivisions` 分割する均等分割」と等価なので、1 回確保 + fill に変更（Numba）。
- [x] **`np.linalg.norm` 回避（平方ノルム比較）**
...
- [x] ステップ 1: 接続エッジ生成をベクトル化（最重要）
- [x] ステップ 2: 2 パス化（count→fill）で `concatenate` と小配列確保を消す
- [x] ステップ 3: auto center の式変形 + `scale==1` 高速経路
