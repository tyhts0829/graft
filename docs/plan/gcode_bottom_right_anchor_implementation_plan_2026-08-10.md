...
production code、設定、テスト、文書の変更は、本計画への再承認後に行う。
...
A5 は `W=148`, `H=210`, 現行 `origin=(154.019, 14.195)` で正しく出力できている。
...
したがって、設定の正本とすべき物理点は次の紙右下アンカーである。
...
| 紙 | 実効 X offset | 右下 machine 座標 | 現行との差 |
|---|---:|---:|---:|
...
固定左下 offset を表す現在の `origin` を廃止し、物理的に合わせる点を直接表す。
...
- [ ] `/Users/tyhts0829/Documents/Grafix/src/grafix/core/gcode_params.py`
  - [ ] `origin` を削除し、有限実数ペア `paper_bottom_right_mm` を追加する。
  - [ ] 既定値を `(302.019, 14.195)` とする。
  - [ ] `canvas_height_mm` を削除する。
  - [ ] `y_down` と右下アンカーの座標契約を NumPy スタイル docstring に記載する。

### 5.2 runtime config

- [ ] `/Users/tyhts0829/Documents/Grafix/src/grafix/core/runtime_config.py`
  - [ ] required key/parser/GCodeParams 構築を `paper_bottom_right_mm` へ移行する。
  - [ ] `origin` と `canvas_height_mm` を旧 key として受け付けない。
- [ ] `/Users/tyhts0829/Documents/Grafix/src/grafix/resource/default_config.yaml`
  - [ ] 新しいアンカー値と Y 反転の意味を記載する。
- [ ] `/Users/tyhts0829/Documents/Grafix/.grafix/config.yaml`
  - [ ] A5 から得た右下アンカー `(302.019, 14.195)` を設定する。
- [ ] config `version: 1` は既存運用どおり据え置く。

### 5.3 G-code exporter

- [ ] `/Users/tyhts0829/Documents/Grafix/src/grafix/export/gcode.py`
  - [ ] `_canvas_to_machine_xy()` を右下アンカー相対の式へ変更する。
  - [ ] X は符号を変えず、Y だけ `y_down` に従って符号を変える。
  - [ ] `canvas_size` の W/H を両軸の配置に使用する。
  - [ ] clip、量子化、bed 検証、stroke 最適化の順序は変えない。

### 5.4 呼び出しと既存テストの移行

- [ ] repository 内の `GCodeParams(origin=...)` と `canvas_height_mm=...` を新契約へ移行する。
- [ ] identity 変換を使う exporter test では、canvas `(W,H)` に対して
  `paper_bottom_right_mm=(W,H)`, `y_down=false` を明示する。
- [ ] 旧 key/property/constructor 引数を残さない。

### 5.5 回帰テスト

- [ ] `/Users/tyhts0829/Documents/Grafix/tests/export/test_gcode.py`
  - [ ] A5 の既知 G-code XY が現在と一致する。
  - [ ] A4 の右下が A5 と同じ `(302.019,14.195)` になる。
  - [ ] A4 の全 X 座標が現状より62 mm左へ移る。
  - [ ] 任意の複数 W/H で canvas 右下が常に同じ machine anchor になる。
  - [ ] 非対称な四隅/点で、Xは非反転、Yだけ反転することを直接検証する。
  - [ ] `paper_bottom_right_mm` の不正tuple・非有限値を拒否する。
- [ ] `/Users/tyhts0829/Documents/Grafix/tests/core/test_runtime_config.py`
  - [ ] default/project config の新アンカーを検証する。
  - [ ] 旧 `origin` / `canvas_height_mm` を拒否する。
- [ ] `/Users/tyhts0829/Documents/Grafix/tests/export/test_capture_service.py`
  - [ ] capture 経路と manifest effective config に新アンカーが保持される。
- [ ] interactive export worker の GCodeParams snapshot test を新 field へ移行する。
- [ ] capture manifest `schema_version: 3` は据え置く。既存の `canvas_size` と
  effective config のアンカーから変換を再構成できるためである。

### 5.6 文書

- [ ] `/Users/tyhts0829/Documents/Grafix/architecture.md` に canvas/machine 軸と右下アンカー契約を追記する。
- [ ] 専用 migration 文書へ、旧 `origin` から右下アンカーへの換算式と破壊的変更を記載する。
- [ ] 旧中央合わせ計画を実装していないことを明記した状態で維持する。
...
- [ ] `mypy src/grafix`。
- [ ] 既存成果物を上書きせず、一時出力で次を確認する。
  - [ ] A5 alignment の G-code が現行と同じ XY bounds/座標になる。
  - [ ] A4 alignment の XY bounds が X 方向だけ `-62 mm` 移動する。
  - [ ] A5/A4 とも右側マーカーと下側マーカーが同じ machine 座標へ揃う。
