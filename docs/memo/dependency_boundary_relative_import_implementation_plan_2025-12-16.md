...
- さらに、`ast.ImportFrom` で `modules.add(node.module)` しかしていないため、次の形も見逃す。
  - 例: `from src import export`（`node.module == "src"` なので `"src.export"` を拾えない）
- `src/` 配下には `from .foo import ...` の相対 import が既に多数ある（相対 import 自体を禁止する方向は現実的ではない）。

## ゴール（成功条件）

- `src/core` で次が書かれたらテストが落ちる:
  - `from ..export import ...` / `from .. import export`
  - `from ..interactive import ...` / `from .. import interactive`
  - `from src import export` / `from src import interactive`
- 既存の “同一層内” 相対 import（例: `src/core/**` の `from .key import ...`）では落ちない。
...
- ファイルパスから “現在モジュール名” を決める:
  - 例: `repo_root/src/core/pipeline.py` → `src.core.pipeline`
  - 例: `repo_root/src/core/__init__.py` → `src.core`（`.__init__` は落とす）
- `ast.ImportFrom` を次のルールで “検査対象モジュール名の集合” に変換する:
  ...
  - 追加で、`from X import Y` の “Y 側” も候補として足す（`Y != "*"` のとき）:
    - `base + "." + name`（`from src import export` → `src.export` を拾うため）
...
- [ ] `current_module="src.core.pipeline"` + `from ..export import svg` → `{"src.export", "src.export.svg"}` を含む
- [ ] `current_module="src.core.pipeline"` + `from .. import interactive` → `{"src.interactive"}` を含む
- [ ] `current_module="src.core.pipeline"` + `from src import export` → `{"src.export"}` を含む
- [ ] `current_module="src.core.parameters.resolver"` + `from . import context` → `{"src.core.parameters.context"}` を含む
- [ ] `from ..export import *` は `{"src.export"}` のみ（`*` は展開しない）
- [ ] 解決不能な相対 import（例: `current_module="src.core"` で `from ...export import svg`）は AssertionError（もしくは明示メッセージ）になる
...
- `from src import export` を「境界違反として検出する」方針で問題ないか（意図どおりなら穴が一段減る）。
- 非パッケージ扱いのサブディレクトリ（`__init__.py` が無い場所）についても、境界検査のため “パス由来の擬似モジュール名” で解決してよいか。
...
- 依存境界ルール自体の拡張（`src/interactive` など別領域の追加）
