...
- ParamState: 各 ParameterKey に紐づく状態（override, ui_value, min, max, cc）。GUI の保存・復元単位。min/max は UI レンジとしてのみ利用する。
...
- マージ: frame_params のログを ParamStore に反映する処理。未登録の ParameterKey は新規に ParamState を作成し、`override=False`・`ui_value=base_value`（正規化後）・`min/max=ParamMeta.ui_min/ui_max` で初期化し、既存キーは上書きせず保持する。GUI 側はこの結果をもとに行を生成・更新する。
