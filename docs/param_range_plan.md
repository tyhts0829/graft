...
何を: ParamMeta.min/max を「GUI スライダーの推奨レンジ」に限定し、描画値のクランプはしない方針へ修正する計画をまとめる。
なぜ: min/max を実値クランプに使うと、canvas サイズ依存の座標や振幅が想定外に潰れ、UI 用レンジ指定が描画を壊すため。

## 0. ゴール

- ParamMeta.min/max は UI スライダーの初期レンジとしてのみ使用する。
- 値解決（effective）は min/max でクランプしない（opt-in フラグも持たない）。
- 既存メタ定義（circle など）を「スライダー推奨レンジ」に見直し、描画を制限しない。
- テストを追加し、min/max が描画値をクランプしないことを担保する。

## 1. 方針

- resolver から clamp 処理を外し、量子化のみ維持。clamp オプションは導入しない。
- ParamMeta の min/max を `ui_min/ui_max` へリネームし、「UI 用」であることを明確にする。
...
- [x] `meta.py`: min/max を `ui_min/ui_max` にリネームし、UI 用であることを明記（clamp フラグは持たない）。
...
- [ ] ドキュメント更新: `parameter_gui_plan.md` に min/max の意味変更を追記。
...
- clamp がデフォルト無効になり、min/max は UI 用であることがコードと docstring で明示されている。
