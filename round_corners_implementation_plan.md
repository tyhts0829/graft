...
- [ ] 各頂点 `V`（開曲線は端点除く、閉曲線は全点）について:
  ...
  - [ ] 角度 `phi = angle(d1, d2)` を計算し、`phi≈0`（直線）や `phi≈π`（Uターン）ではスキップする
...
- [ ] effect 名は `round_corners` と `fillet` のどちらが良い？
- [ ] `segments` は「円弧の分割数（角ごと）」で良い？（別案: `segments_per_quadrant`）
- [ ] `closed` 引数は必要？（不要なら「auto 判定のみ」で固定してシンプル化する）
- [ ] 半径が大きいときの挙動: 「自動で最大半径へクランプ」で良い？それとも「その角だけ角丸しない」が良い？
