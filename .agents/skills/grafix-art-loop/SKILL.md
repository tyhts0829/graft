...
1. **初期化する。** `n`未指定時は3とし、次を実行する。
   ...
   表示された `run_dir` をこのrunの作業用出力先にする。
...
5. **一括表示する。** 次を一度実行する。
   ...
   contact sheet は `data/output/png/codex_generated/<run_id>_contact_sheet.png` に保存される。
...
1. 画像、コード、ログ、一時物を含むrun成果物は現在の `run_dir` 配下だけに置く。例外として、contact sheetを `data/output/png/codex_generated/<run_id>_contact_sheet.png` に保存し、確定した最終PNGを同directoryの `<run_id>.png` にも複製する。
...
- `data/output/png/codex_generated/<run_id>_contact_sheet.png` にcontact sheetがある。
