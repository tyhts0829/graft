...
- [ ] `DrawWindowSystem` に録画状態を追加して V トグルを実装
  ...
  - [ ] 録画中の `t` は録画タイムラインへ切り替える（`t = t0 + frame_index / fps`）
  - [ ] `draw_frame()` の末尾で「録画中なら framebuffer を read して 1 フレーム書く」→ `frame_index += 1`
  - [ ] start/stop 時に、録画 fps / 書き込んだ frames / 動画秒数（`frames/fps`）をログ出力する（実時間と一致しないため）
