...
- [ ] (1-1) `grafix` を typed package として宣言する（`py.typed` を追加する）
  - A: はい（推奨）。`grafix` 内の解析が有効になる
  - B: いいえ。今回は `sketch/readme.py` 側の `# type: ignore[import-untyped]` 等で黙らせる
- [ ] (1-2) `py.typed` を配布物（wheel/sdist）へ確実に含めるため、`pyproject.toml` に package-data を追加する
  - A: はい（推奨）。`pip install .` でも確実に反映される
  - B: いいえ（当面 editable install 前提で進める）
- [ ] (1-3) `py.typed` 追加後に新規に出る可能性がある mypy エラーの扱い
  - A: 今回は「警告を消す」まで（新規エラーは別途）
...
- [ ] `mypy sketch/readme.py` で当該 warning が消えることを確認する
- [ ] `PYTHONPATH=src mypy sketch/readme.py` でも同様に確認する（ローカル参照の確認）
- [ ] （必要なら）インストールし直す
...
- [ ] `py.typed` 追加で mypy の解析範囲が増え、想定外にノイズが増えないか（増える場合は別計画化）
