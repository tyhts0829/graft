...
`src/graft/core/geometry.py` の `Geometry` は `(op, inputs, args)` を持つ不変ノードで、`id` は内容署名（content signature）。  
`draw(t)` は配列ではなく Geometry レシピを返す。

2) **実体配列は realize で遅延生成し、GeometryId でキャッシュする**  
`src/graft/core/realize.py` の `realize(geometry)` が `Geometry -> RealizedGeometry` を評価する。  
`RealizedGeometry` は `coords(float32, Nx3)` と `offsets(int32, M+1)` を持ち、配列は writeable=False で固定される（`src/graft/core/realized_geometry.py`）。

3) **描画スタイル（色・線幅）は Geometry から分離し Layer に載せる**  
`src/graft/core/layer.py` の `Layer` が `Geometry + (color/thickness)` を束ねる。  
...
user sketch (main.py の draw(t))
  |
  v
src/graft/api            : G/E/L/run（書き味の層）
  |  Geometry.create() を呼ぶ / resolve_params() を呼ぶ
  v
src/graft/core           : Geometry / realize / registries（ドメイン核）
  ^\
  | \__ src/graft/core/primitives, src/graft/core/effects : @primitive/@effect で登録（実装の“プラグイン”）
  |
  +--> src/graft/core/parameters : ParamStore / parameter_context / resolve_params（入力解決）
  |
  v
src/graft/core/pipeline   : Scene 正規化 / Layer style 解決 / realize（出力・描画の共通パイプライン）
  |\
  | \__ src/graft/export        : RealizedLayer -> ファイル（ヘッドレス出力、現状はスタブ）
  |
  v
src/graft/interactive     : pyglet + ModernGL + Parameter GUI（対話プレビュー）
        |
        +--> src/graft/interactive/parameter_gui : ParamStore 編集 UI（pyimgui）
```

重要な「呼び出し順」は次。

1. `src/graft/api/run.py:run()` が `ParamStore` とウィンドウサブシステムを作る
2. `src/graft/interactive/runtime/window_loop.py:MultiWindowLoop.run()` がフレームループを回す
...
4. `realize_scene()`（`src/graft/core/pipeline.py`）が
   - `normalize_scene(draw(t))`（`src/graft/core/scene.py`）で Layer 列にし
...
5. `DrawWindowSystem` が
   - `build_line_indices(offsets)`（`src/graft/interactive/gl/index_buffer.py`）でインデックスを生成し
   - `DrawRenderer.render_layer(...)`（`src/graft/interactive/gl/draw_renderer.py`）へ描画依頼する
...
- 実装: `src/graft/core/realized_geometry.py`
- 形:
...
`src/graft/core/primitive_registry.py` と `src/graft/core/effect_registry.py` が op 名 → 実体関数を保持する。
...
- `src/graft/api/primitives.py` が `graft.core.primitives.circle/polygon/polyhedron` を import して登録させる
- `src/graft/api/effects.py` が `graft.core.effects.scale/rotate/fill` を import して登録させる

この方式により、`from graft.api import G, E` した時点で “組み込み op が使用可能” になる。

### 5.3 新しい primitive/effect を追加する方法（最短）

1. `src/graft/core/primitives/` か `src/graft/core/effects/` に新モジュールを追加
2. `@primitive(meta=...)` または `@effect(meta=...)` で関数を登録
3. 起動時に import されるようにする（どちらか）
   - `src/graft/api/primitives.py` / `src/graft/api/effects.py` に import を追加する（組み込みとして常時有効化）
   - あるいはスケッチ側でそのモジュールを import する（必要時だけ有効化）

## 6. realize（評価）とキャッシュ

実装: `src/graft/core/realize.py`
...
実装: `src/graft/core/parameters/context.py`
...
実装: `src/graft/core/parameters/resolver.py`
...
実装: `src/graft/core/parameters/store.py:store_frame_params()`
...
- Global style: `src/graft/core/parameters/style.py`
...
- Layer style: `src/graft/core/parameters/layer_style.py`
  - `LAYER_STYLE_OP="__layer_style__"`
  - `line_thickness`, `line_color`
  - `realize_scene()`（`src/graft/core/pipeline.py`）が Layer ごとにエントリを確保し、override=True の場合だけ上書きして描画する

## 8. Parameter GUI（pyimgui）アーキテクチャ

GUI は「描画（imgui）」「データ変換（純粋関数）」「store 反映」を分離している。

- 入口（ライフサイクル）: `src/graft/interactive/parameter_gui/gui.py`
  - ImGui context を生成し、毎フレーム `render_store_parameter_table(store)` を呼ぶ
- backend（pyglet 依存）: `src/graft/interactive/parameter_gui/pyglet_backend.py`
  - window 生成、IO 同期、renderer 作成
- store ↔ rows ↔ UI の橋渡し: `src/graft/interactive/parameter_gui/store_bridge.py`
  1) `store.snapshot()` を読む  
  2) `rows_from_snapshot()` で `ParameterRow` に変換（`src/graft/core/parameters/view.py`）  
...
- “純粋なロジック” を集約:
  - `src/graft/core/parameters/view.py` … 値正規化・rows 生成・state 反映 API（imgui 非依存）
  - `src/graft/interactive/parameter_gui/grouping.py` / `group_blocks.py` / `labeling.py` … 表示名・ブロック化
  - `src/graft/interactive/parameter_gui/rules.py` … kind/op ごとの列表示ルール
  - `src/graft/interactive/parameter_gui/widgets.py` … kind→widget の対応（imgui 呼び出しはここに寄せる）

## 9. 描画（ModernGL）パイプライン

### 9.1 シーン正規化 → realize → 描画

実装: `src/graft/core/pipeline.py` と `src/graft/interactive/runtime/draw_window_system.py`
...
2) `parameter_context(store)` の中で `realize_scene(draw, t, defaults)` を呼ぶ  
   - `normalize_scene(draw(t))`（`src/graft/core/scene.py`）
   - `resolve_layer_style(layer, defaults)`（`src/graft/core/layer.py`）
   - layer_style（line_thickness/line_color）の GUI override
   - `realize(geometry)`（`src/graft/core/realize.py`）
3) 各 `RealizedLayer` について
   - `indices = build_line_indices(offsets)`（`src/graft/interactive/gl/index_buffer.py`）
   - `DrawRenderer.render_layer(...)`（`src/graft/interactive/gl/draw_renderer.py`）

### 9.2 GPU レンダラーの構成

- `src/graft/interactive/gl/draw_renderer.py:DrawRenderer`
...
`src/graft/api/run.py` はこのループの “配線” に徹し、

- 描画: `DrawWindowSystem`（`src/graft/interactive/runtime/draw_window_system.py`）
- GUI: `ParameterGUIWindowSystem`（`src/graft/interactive/runtime/parameter_gui_system.py`）
...
- ヘッドレス export の導線（`src/graft/api/export.py:Export` と `src/graft/export/*`）はあるが、実ファイル生成は未実装（スタブ）
