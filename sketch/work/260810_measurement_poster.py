...
POSTER_LINES_META = {
    "line_group": {
        "kind": "choice",
        "choices": POSTER_LINE_GROUPS,
        "display_name": "Line family",
        "description": "Selects the technical line family generated by this node.",
    },
}


@primitive(meta=POSTER_LINES_META)
def measurement_poster_lines(
    *, line_group: str = "primary_lines"
) -> tuple[np.ndarray, np.ndarray]:
    """アーカイブ調の技術プレート線群を濃度別に返す。"""

    data = _archive_groups()
    try:
        index = POSTER_LINE_GROUPS.index(line_group)
    except ValueError as exc:
        raise ValueError(f"Unknown poster line group: {line_group!r}") from exc
...
def _combine_geometries(items: list | tuple):
    geometry = items[0]
    for item in items[1:]:
...
def _value_swatch_geometries() -> tuple:
    ...
    width = (x1 - x0) / 11.0
    geometries = []
    for index, min_spacing in enumerate(VALUE_SWATCH_MIN_SPACING):
        value = index * 10
...
def draw(t: float):
    ...
    grain = G(name="Paper fibers / archival grain").measurement_poster_lines(
        activate=False,
        line_group="paper_fibers",
        key="paper-fibers",
    )
    mist = G(name="Microfilm mist / pale construction").measurement_poster_lines(
        line_group="pale_construction",
        key="microfilm-mist",
    )
    grid = G(name="Fine ruled grids / all panels").measurement_poster_lines(
        line_group="fine_grids",
        key="fine-ruled-grids",
    )
    main = G(name="Technical plate / primary lines").measurement_poster_lines(
        line_group="primary_lines",
        key="technical-plate-lines",
    )
    heavy = G(name="Dense ink / marks and bars").measurement_poster_lines(
        line_group="dense_marks",
        key="dense-ink-marks",
    )
    typography_specs = (
        ("Top panel headers", "top-panel-headers"),
        ("Top panel annotations", "top-panel-annotations"),
        ("Bottom panel headers", "bottom-panel-headers"),
        ("Bottom panel annotations", "bottom-panel-annotations"),
        ("Outer ruler labels", "outer-ruler-labels"),
    ...
    typography = tuple(
        fit_a4(
            E(name=f"Typography fill / {name}").fill(
                angle_sets=2,
                angle=45.0,
                density=TEXT_FILL_DENSITY,
                min_spacing=TEXT_FILL_MIN_SPACING,
                remove_boundary=False,
                key="typography-fill",
                instance_key=key,
            )(geometry)
    ...
    return L(name="Black technical poster / single layer").layer(geometry)
