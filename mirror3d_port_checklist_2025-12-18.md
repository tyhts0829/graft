...
- [ ] 新規ファイル配置: `src/grafix/core/effects/mirror3d.py`（effect 名は関数名 `mirror3d`）
- [ ] 公開 API（引数）方針:
  - A: 旧仕様をそのまま踏襲（`cx,cy,cz` を個別引数のまま、`axis`, `phi0`, `mode`, `group`, `use_reflection`, `mirror_equator`, `show_planes` 等）
  - B: 新プロジェクト寄りに整理（例: `center: vec3` 化、`source_side` 整理など）※破壊的
- [ ] `source_side` の扱い（旧コードでは実質未使用）:
  - A: 旧 API として残すが挙動は現状踏襲（=内部では使わない）
  - B: 削除する（シンプルだが旧仕様から逸れる）
  - C: “赤道ミラーのソース側選択”として実装し直す（仕様追加・要設計）
- [ ] `mode='polyhedral'` のソース領域:
  - A: 旧実装通り「正の八分体（x>=cx,y>=cy,z>=cz）」でクリップ
  - B: 将来予定の「球面三角形（群の基本領域）」へ変更（大きめの設計/実装）
- [ ] `mode='polyhedral'` の反射倍化:
  - A: 旧実装通り代表反射（y=0）で倍化
  - B: 反射生成元から群を生成（旧ファイル末尾の未使用関数相当）※変更大
- [ ] UI メタ情報（`ParamMeta` の `ui_min/ui_max`）:
  - `cx,cy,cz` のレンジを旧メタ（0..1000）に寄せるか、他 effect に合わせて ±500 などに寄せるか
...
- [ ] `src/grafix/core/effects/mirror3d.py` を新規作成
- [ ] 先頭 docstring は「効果が何か」の説明にする（effects/ 配下の規約に従い “どこで/なにを/なぜ” は書かない）
- [ ] `mirror3d_meta: dict[str, ParamMeta]` を定義（`mode/group` は `kind=\"choice\"`）
- [ ] `@effect(meta=mirror3d_meta)` で `mirror3d(...) -> RealizedGeometry` を実装
  - 入力が空なら空ジオメトリを返す（他 effect と同様）
  - `inputs[0]` を対象とする（基本 1 入力）
- [ ] 旧実装の内部ヘルパ（`_unit`, 回転/反射, 半空間クリップ, 行列列挙, dedup, show_planes）を新プロジェクト向けに移植
...
- [ ] `src/grafix/api/effects.py` に `from grafix.core.effects import mirror3d as _effect_mirror3d  # noqa: F401` を追加
- [ ] `E.mirror3d(...)` が `AttributeError` にならないことを確認

## 4) テスト追加（旧仕様の安全柵）

- [ ] `tests/core/effects/test_mirror3d.py` を追加
- [ ] `azimuth` の最小ケース（`n_azimuth=1`）で 2 倍（+必要なら `mirror_equator` でさらに倍）になること
- [ ] `azimuth` の `n_azimuth>=2` で `2*n_azimuth` 本（+赤道ならさらに倍）になること（dedup 後の本数で確認）
- [ ] `polyhedral` の `group='T'|'O'|'I'` で「回転群の本数に一致する（または dedup 後に期待本数）」を確認
- [ ] `show_planes=True` で出力本数が増えること（面の十字線が追加されること）
...
- [ ] `PYTHONPATH=src pytest -q tests/core/effects/test_mirror3d.py`
