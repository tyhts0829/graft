...
- [x] `src/graft/` → `src/grafix/` に移動（`git mv` 相当）
- [x] 先頭ヘッダコメント等のパス表記（例: `src/graft/...`）を `src/grafix/...` に更新

## 3) import / 参照の更新

- [x] `src/` 内の `import graft` / `from graft ...` を `grafix` に置換
...
- [x] `tests/architecture/test_dependency_boundaries.py` の `src/graft` や `graft.*` を `grafix` に更新
- [x] `tests/**` の import パス期待値があれば更新

## 5) ドキュメント/運用手順の更新

- [x] `README.md` の import 例（`from graft.api ...`）を `grafix` に更新
- [x] `architecture.md` 等の主要ドキュメントの `graft` 参照を更新（0) のスコープに従う）
- [x] `docs/memo/performance.md` のパス/ログ例（`src/graft/...`, `[graft-perf]`）を `grafix` に更新
- [x] `AGENTS.md` の Build/Test/Style（例: `src/graft/`, `mypy src/graft`）を `grafix` に更新
