...
- [ ] `inputs` が空のときの bypass の返り値を決める（空ジオメトリ / 例外 / 既存処理）。
- [ ] `inputs` が複数の effect（将来）で bypass が返す対象を決める（基本は `inputs[0]` で良い？）。
- [ ] meta 無し effect は GUI に出さない方針で OK か確認する。
...
- [ ] bypass=True で「戻り値は入力と同一インスタンス」を期待する？（性能/キャッシュの観点）
- [ ] bypass=True のとき、「effect ノード自体を DAG から省略」したい？（API 層での最適化が必要になる）
