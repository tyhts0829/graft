...
PNG は **SVG を正（ソース）**として「SVG→PNG のラスタライズ」で生成する（方式2）。
...
- 画像サイズ: `canvas_size` を基準に **倍率で決める**
  ...
  - ユーザー制御: `run(..., png_scale=...)` で指定できるようにする
...
- [ ] `png_scale` を `run()` からユーザー指定できるようにし、`png_size=(canvas_w*scale, canvas_h*scale)` を決める
...
- [ ] 手動確認（実行→P→生成PNGを開き、解像度=canvas*scale / 背景 / 見た目 を確認）

## 事前に確認したい点（あなた向け質問）

- [ ] ラスタライザは `resvg` 依存で進めて OK？（見つからなければエラー表示）
- [ ] `png_scale` の既定値は 4.0 で OK？（`canvas_size=(300,300)` → `1200x1200`）
- [ ] `png_scale` の指定場所は `run(..., png_scale=...)` で良い？（Parameter GUI からの変更は今回は不要？）
