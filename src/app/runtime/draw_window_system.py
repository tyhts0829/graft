...
from src.parameters.style import (
...
class DrawWindowSystem:
    ...
    def draw_frame(self) -> None:
        ...
        with parameter_context(self._store, cc_snapshot=None):
            ...
            # render_scene は:
            # - draw(t) の戻り値を normalize_scene で Layer 列へ正規化
            # - resolve_layer_style で None 欠損を effective_defaults で埋める
            # - realize → index 生成 → renderer.render_layer(...) を繰り返す
            # という一連の描画パイプラインを実行する。
            render_scene(self._draw, t, effective_defaults, self._renderer)
