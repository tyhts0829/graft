"""
どこで: `src/effects/collapse.py`。collapse effect の実体変換。
何を: 各線分を細分化し、サブセグメントごとに直交方向へランダム変位させて「崩し」を作る。
なぜ: 旧プロジェクト由来の effect を、現行の RealizedGeometry/effect チェーン仕様へ移植するため。
"""
