"""
どこで: `src/effects/subdivide.py`。subdivide effect の実体変換。
何を: 各ポリラインの全セグメントへ中点挿入を繰り返し、頂点密度を増やす。
なぜ: 旧プロジェクトの `subdivide` を現行の RealizedGeometry/effect チェーン仕様へ移植するため。
"""
